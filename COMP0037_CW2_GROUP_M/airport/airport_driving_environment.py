...
class AirportDrivingEnvironment(Env):
    ...
    def __init__(self, airport_map):
        ...
        # Current coordinates, used for stepping
    ...
    def reset(self, start_state = None):
        self._current_coords = start_state
        self._steps_since_reset = 0
    ...
    def step(self, action):
        ...
        # If we've reached the maximum number of steps, return
        self._steps_since_reset += 1
        
        if self._steps_since_reset > 1000:
            return None, -10000, True, False
            
        # Get the distribution of rewards, actions, etc.
        s_prime, r, p = self. next_state_and_reward_distribution(self._current_coords, action)
        
        # Now handle the terminate action case
        if action is DrivingActionType.TERMINATE:
            #assert(self._airport_map.cell(self._current_coords[0], self._current_coords[1]).is_terminal())
            return None, r[0], True, True
        ...
        # Set the new state
        self._current_coords = s_prime[k].coords()
        
                
        #print(f'next_coords={self._current_coords}({s_prime[k].cell_type()})')
        
        return self._current_coords, r[k], False, []
    
    def pick_random_start(self, num_attempts = 10000):

        max_x = self._airport_map.width() - 1
        max_y = self._airport_map.height() - 1 
        
        # Iterate to try to find a not obstructed cell
    ...
    def next_state_and_reward_distribution(self, s, a, print_cell= False):
        ...
        for i in range(-1,2):
            ...
            if (new_x < 0) or (new_x >= self._airport_map.width()) \
                ...
                r.append(-math.sqrt(delta[0] * delta[0] + delta[1] * delta[1]))
                if print_cell:
